...
//Objeto lcd -> "lcd"
LiquidCrystal_I2C lcd(0x27,16,2);     //SCL -> A5 y SDA -> A4
...
float corrienteBase;         //Parámetro de control de corriente
...
estadosBrazo estadoBrazo = CODO;    //Se inicializa en el codo
...
float get_corriente(int n_muestras)
{
  ...
  corriente=corriente/n_muestras;                           //Se calcula la media
  return(corriente);
}
//Funcion parada 
...
if (get_corriente(500) <= (corrienteBase - 0.3) | get_corriente(500) >= (corrienteBase + 0.3)){  //Si se alcanza un límite por encima o por debajo
 
  digitalWrite(en, LOW);           //Parar motor
  Serial.println("  PARADA"); 
  estadoMov = PARA;                //Se lleva al estado de parada
}
}
...
void setup()
{
  ...
  //Inicializar comunicación serial (115200 baudios obligatorio)
  Serial.begin(115200);
  ...
  if(myVR.clear() == 0){
    Serial.println("Recognizer cleared.");                           //Sensor de voz detectado y correcto
  }else{
    Serial.println("Not find VoiceRecognitionModule.");              //Sensor de voz no detectado o incorrecto
    Serial.println("Please check connection and restart Arduino.");
    while(1);                                                        //El código no se ejecuta hasta que se solucione el error
  }
  
  //Mensajes de carga de comandos paa cada comando guardado
  ...
  corrienteBase = get_corriente(1000);    //Solo se ejecuta al principio

  //Posición inicial
  servo_q1.write(150,10);      //Primer parámetro = ángulo, segundo parámetro = velocidad de giro (0-255)
  servo_q2.write(30,30);
  servo_q3.write(150,30);
}
...
void loop()
{
  ...
  //Actulizacion lcd
  lcd.clear();
  //Primera linea (estados del brazo)
  switch (estadoBrazo){
    case CODO:
      lcd.println("CODO            ");      //Los espacios permiten que la pantalla se vea de forma clara
      break;
    case MUNYECA:
      lcd.println("MUNYECA         ");
      break;
  ...
  //Segunda linea (estados de movimiento)
  lcd.setCursor(0, 1);
  switch (estadoMov){
    case ABRIR:
      lcd.println("ABRIR            ");
